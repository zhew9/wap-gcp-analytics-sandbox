...
### My brief comparison of Write-Audit-Publish options on different data warehouses (as of 2025-08) 
  ...
  * For **`Databricks`**: it's a bit pretty unclear on what their official WAP implementation is for Delta Lake (so I have to do a bit of guess work) - but they also support Iceberg so that is always an option: 
    * The main option seems to be `SHALLOW CLONE` ing the live tables to the audit environment and then after transforming/auditing the clones, `MERGE INTO` the shallow clones back into their original base table as the Publish step w/ a [potential example here](https://docs.databricks.com/aws/en/delta/clone#use-clone-for-short-term-experiments-on-a-production-table). However, it's unclear if the `MERGE INTO` operation is like that of Iceberg branches (where the merge is only a metadata/pointer operation) or a standard, data-moving merge operation - if it's data-moving, then there will still be a physical data transformation that you'd be performing outside of your audit environment (which still leaves a bit of risk) - though the logic is vetted and the merge is idempotent:
      * Hopefully someone with more experience implementing WAP on Databricks can clear up if the `MERGE INTO` for `SHALLOW CLONE`s is just a metadata/pointer operation or not, and if there is another WAP option in Databricks
      * The potential data-moving `MERGE INTO` isn't so much of a problem for cost: depending on their shallow clone implemention, their combined clone transformations + merge cost might be equivalent to the clone transformation costs on BigQuery/Snowflake/Iceberg. It's just that it has some risk.
