-- models/staging/deduplication/stg_retail__customers.sql
...
with source as (
    ...
    select 
        *
    from {{ source('boardgame_retail_raw', 'customers') }}
...
deduped as (
    select
        *
    from source
    qualify row_number() over (partition by customer_id order by date_modified desc) = 1
)
