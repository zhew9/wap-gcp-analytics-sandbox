-- models/staging/deduplication/stg_retail__channels.sql
...
with source as (
    ...
    select 
        *
    from {{ source('boardgame_retail_raw', 'channels') }}
...
deduped as (
    select
        *
    from source
    qualify row_number() over (partition by channel_id order by date_modified desc) = 1
)
