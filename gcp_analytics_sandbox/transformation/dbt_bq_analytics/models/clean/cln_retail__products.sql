-- models/staging/deduplication/stg_retail__products.sql
...
with source as (
    ...
    select 
        *
    from {{ source('boardgame_retail_raw', 'products') }}
...
deduped as (
    select
        *
    from source
    qualify row_number() over (partition by product_id order by date_modified desc) = 1
)
