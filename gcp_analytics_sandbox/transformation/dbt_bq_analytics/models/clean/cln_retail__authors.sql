-- models/staging/deduplication/stg_retail__authors.sql
...
with source as (
    ...
    select 
        *
    from {{ source('boardgame_retail_raw', 'authors') }}
...
deduped as (
    select
        *
    from source
    qualify row_number() over (partition by author_id order by date_modified desc) = 1
)
