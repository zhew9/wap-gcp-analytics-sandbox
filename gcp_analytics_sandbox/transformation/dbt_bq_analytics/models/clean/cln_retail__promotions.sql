-- models/staging/deduplication/stg_retail__promotions.sql
...
with source as (
    ...
    select 
        *
    from {{ source('boardgame_retail_raw', 'promotions') }}
...
deduped as (
    select
        *
    from source
    qualify row_number() over (partition by promotion_id order by date_modified desc) = 1
)
