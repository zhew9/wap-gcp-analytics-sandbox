-- models/marts/bridge_product_publishers_history.sql
...
{{
    config(
        ...
        cluster_by=["publisher_sk","product_sk"],
        unique_key='product_publisher_sk',
...
with source_bridge as (
    ...
    select
        ...
        removed_at,
        removal_reason
    from {{ ref('stg_retail__products_publishers_bridge') }}
    {% if is_incremental() %}
    where
...
product_and_publisher_versions_joined as (
    select
        {{ dbt_utils.generate_surrogate_key(['products_history.product_sk', ' publishers_history.publisher_sk']) }} as product_publisher_sk,
        -- The version-specific surrogate keys
        products_history.product_sk,
        publishers_history.publisher_sk,
    ...
    left join publishers_history
        on source_bridge.publisher_id = publishers_history.publisher_id
        and source_bridge.added_at >= publishers_history.valid_from_ts
        and source_bridge.modified_at < coalesce(publishers_history.valid_to_ts, '9999-12-31')
)

select * from product_and_publisher_versions_joined