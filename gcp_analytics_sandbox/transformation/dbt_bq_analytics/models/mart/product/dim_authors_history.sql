-- models/marts/product/dim_authors_history.sql
...
{{
    config(
        ...
        unique_key='author_sk',
        on_schema_change='fail',
    )
}}

-- {{source('write_audit_publish', 'audit_environment_enabled')}}
with updated_author_snapshots as (
    select
        *
    from {{ ref('stg_snapshot_retail__authors') }}
