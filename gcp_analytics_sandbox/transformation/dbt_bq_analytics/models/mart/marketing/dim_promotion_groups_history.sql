{{
    config(
        ...
        cluster_by=["updated_at","promotion_group_id"],
        unique_key='promotion_group_sk',
        on_schema_change='fail',
    )
}}

-- {{source('write_audit_publish', 'audit_environment_enabled')}}
with updated_promotion_group_snapshots as (
    select 
        * 
    from {{ ref('stg_snapshot_retail__promotion_groups') }}
...
final as (
    select
        ...
        dbt_scd_id as promotion_group_sk,
        ...
        is_current_record
    from updated_promotion_group_snapshots
)
